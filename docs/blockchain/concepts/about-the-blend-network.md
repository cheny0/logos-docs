...
- Reward distribution: Starting immediately after the epoch when activity messages are submitted, rewards are gradually distributed to active service nodes. Each block includes one [Mantle](../../get-started/glossary.md#mantle) transaction that can distribute rewards to up to four nodes.
